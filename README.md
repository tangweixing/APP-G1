...
底层调 `unitree_sdk2_python/example/g1/high_level/g1_arm_action_example.py`，传 `网络接口 + actionId` 执行。G1 共 **16 个预设动作（ID 0–15）**：

| ID | 动作 | 工具 | 状态 |
|----|------|------|------|
| 1 | 握手 shake hand | `handshake` | ✅ |
| 2 | 击掌 high five | `bixin` | ✅ |
| 3 | 拥抱 hug | `yongbao` | ✅ |
| 6 | 你好 face wave | `nihao` | ✅ |
| 7 | 飞吻 left kiss | `qin` | ✅ |
| 10 | 要饭 hands up | `yaofan` | ✅ |
| 11 | 迪迦 x-ray | `dijia` | ✅ |
| 0 | 释放手臂 release arm | — | 工具内自动调用 |
| 4 | 拜拜 high wave | — | ⬜ 待接入 |
| 5 | 鼓掌 clap | — | ⬜ 待接入 |
| 8 | 比心 heart | — | ⬜ 待接入 |
| 9 | 瞭望 right heart | — | ⬜ 待接入 |
| 12 | 右手抬 right hand up | — | ⬜ 待接入 |
| 13 | 拒绝 reject | — | ⬜ 待接入 |
| 14 | 右飞吻 right kiss | — | ⬜ 待接入 |
| 15 | 双手飞吻 two-hand kiss | — | ⬜ 待接入 |

> ⚠️ 已知问题：现有 `bixin` 工具实际用的是 ID=2（击掌），真正的「比心」是 ID=8，未接入。

新增一个动作工具只需在 `src/mcp/tools/` 下复制 `nihao/` 改 `action_id`，并在 `src/mcp/mcp_server.py` 注册。
...
- `tang/WK/unitree_sdk2_python/example/g1/high_level/g1_arm_action_example.py` — 16 个动作定义
