...
def _kill_camera_occupants():
    """清理占用 RealSense 相机的进程（拍照前调用）。

    解决 "xioctl(VIDIOC_S_FMT) failed, errno=16 Device or resource busy"：
    当 g1_gesture_control.py / face.py 等进程占用相机时，RealSense pipeline.start
    会报 "Device or resource busy"。这里在拍照前先释放设备。

    注意：不能 kill g1_face_greet.py 自身，否则会导致自杀后拍照无法完成。
    """
    import subprocess
    import glob
    import os
    ...
    # 1. kill 其他占用相机的脚本（不含 g1_face_greet.py 自身）
    for script in ['g1_gesture_control.py', 'face.py']:
        try:
...
def capture_photo_realsense(max_retries=4):
    """使用 RealSense D435i 深度相机拍照一张，返回 numpy array 或 None。

    带重试退避：每次尝试前清理占用进程；失败时多次重试并逐步加长等待，
    应对"占用进程刚被 kill -9、uvcvideo 尚未释放设备"的瞬态 EBUSY。
    （不重置 USB——实测 USBDEVFS_RESET 会破坏 RealSense depth 流。）
    """
    last_err = None

    for attempt in range(1, max_retries + 1):
        print(f"[相机] 拍照前清理可能占用相机的进程... (第 {attempt}/{max_retries} 次)")
        ...
        try:
            ...
            last_err = str(e)
            print(f"❌ 相机启动失败 (第 {attempt}/{max_retries} 次): {e}")
            wait = 2 * attempt  # 2/4/6/8s 退避，给 uvcvideo 释放设备的时间
            print(f"⏳ 等待 {wait}s 后重试...")
            time.sleep(wait)
        except Exception as e:
            last_err = str(e)
            print(f"❌ 拍照失败 (第 {attempt}/{max_retries} 次): {e}")
            time.sleep(2)
        finally:
            if started:
                try:
    ...
    print(f"❌ 拍照重试 {max_retries} 次仍失败: {last_err}")
    return None
...
class FaceGreet:
    ...
    def run(self):
        # SIGTERM → 优雅停止：置 running=False，让拍照循环在两轮之间退出
        # (capture_photo_realsense 的 finally 已 stop pipeline，不会遗留占用)
        def _on_sigterm(*_):
            self.running = False
