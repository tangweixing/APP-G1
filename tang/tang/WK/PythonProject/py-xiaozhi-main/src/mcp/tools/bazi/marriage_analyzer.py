...
class MarriageAnalyzer:
    ...
    def _predict_marriage_age(
        ...
        if day_gan in CHANGSHENG_TWELVE:
            ...
            if changsheng_state in ["长生", "帝旺", "建禄"]:
                ...
                factors["detailed_analysis"].append(f"日主{changsheng_state}，自信有魅力")
            elif changsheng_state in ["墓", "死", "绝"]:
                factors["score"] += 8
                factors["late_signs"].append(f"日主在日支{changsheng_state}")
                factors["detailed_analysis"].append(f"日主{changsheng_state}，需要时间积累")
    ...
    def _get_favorable_marriage_years(
        ...
        if day_zhi in ZHI_RELATIONS:
            ...
            if liuhe_zhi:
                favorable_branches.append({"zhi": liuhe_zhi, "reason": "日支六合", "priority": "高"})
        ...
        for sanhe_combo, element in ZHI_SAN_HE.items():
            if day_zhi in sanhe_combo:
                ...
                for zhi in sanhe_combo:
                    if zhi != day_zhi:
                        favorable_branches.append({"zhi": zhi, "reason": f"三合{element}局", "priority": "高"})
        ...
        for sanhui_combo, element in ZHI_SAN_HUI.items():
            if day_zhi in sanhui_combo:
                for zhi in sanhui_combo:
                    if zhi != day_zhi:
                        favorable_branches.append({"zhi": zhi, "reason": f"三会{element}方", "priority": "中"})
        
        # 4. 桃花星 - 感情运佳
        taohua_zhi = TAOHUA_XING.get(day_zhi, "")
        if taohua_zhi:
            favorable_branches.append({"zhi": taohua_zhi, "reason": "桃花星", "priority": "中"})
        
        # 5. 驿马星 - 变动之年，适合结婚
        yima_zhi = YIMA_XING.get(day_zhi, "")
        if yima_zhi:
            favorable_branches.append({"zhi": yima_zhi, "reason": "驿马星", "priority": "中"})
        
        # 6. 月支相关的有利年份
        if month_zhi in ZHI_RELATIONS:
            month_liuhe = ZHI_RELATIONS[month_zhi].get("六", "")
            if month_liuhe:
                favorable_branches.append({"zhi": month_liuhe, "reason": "月支六合", "priority": "中"})
        
        # 7. 年支相关的有利年份
        if year_zhi in ZHI_RELATIONS:
            year_liuhe = ZHI_RELATIONS[year_zhi].get("六", "")
            if year_liuhe:
                favorable_branches.append({"zhi": year_liuhe, "reason": "年支六合", "priority": "低"})
    ...
    def _analyze_marriage_obstacles(self, eight_char_data: Dict[str, Any]) -> List[str]:
        ...
        # 6. 夫妻星受克分析
        marriage_star_analysis = self._analyze_marriage_star(eight_char_data, 1)  # 先用男性分析
    ...
    def _comprehensive_star_analysis(self, marriage_stars: List[Dict[str, Any]], day_gan: str, gender: int) -> Dict[str, Any]:
        ...
        # 分析分布情况
        distribution_desc = f"共{len(marriage_stars)}个夫妻星，分布在{len(set(positions))}个位置"
        ...
        return {
            ...
            "potential": potential,
            "suggestions": suggestions if suggestions else ["夫妻星配置良好，感情发展顺利"]
        }
