...
async def youshou_function(args: dict) -> str:
    """
    触发右手抬起动作（ID=12），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：right hand up（右手抬起）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=12动作（右手抬起）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or f"ID=12动作（右手抬起）执行成功（无输出）"
        return f"ID=12动作（右手抬起）执行成功：{output}"
