...
async def youfeiwen_function(args: dict) -> str:
    """
    触发右飞吻动作（ID=14），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：right kiss（右飞吻）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=14动作（右飞吻）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or f"ID=14动作（右飞吻）执行成功（无输出）"
        return f"ID=14动作（右飞吻）执行成功：{output}"
