...
async def dijia_function(args: dict) -> str:
    """
    触发迪迦动作（ID=11），拉起指定路径的g1_arm_action_example.py脚本
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=11动作（迪迦）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or "ID=11动作（迪迦）执行成功（无输出）"
        return f"ID=11动作（迪迦）执行成功：{output}"
