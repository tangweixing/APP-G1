...
class Application:
    ...
    async def _run_application_core(self, protocol: str, mode: str):
        ...
        try:
            ...
            # 自动打开音频通道并开始监听
            logger.info("自动打开音频通道并开始监听...")
            if self.protocol:
                await self.protocol.open_audio_channel()
                # 等待音频通道打开
                await asyncio.sleep(1)
                # 设置为监听状态
                logger.info("准备设置设备状态为LISTENING...")
                await self._set_device_state(DeviceState.LISTENING)
                logger.info(f"设备状态已设置为: {self.device_state}")
    ...
    async def _initialize_audio(self):
        ...
        try:
            ...
            # 获取音频配置
            use_g1_audio = self.config.get_config("AUDIO_OPTIONS.USE_G1_AUDIO", False)
            g1_network_interface = self.config.get_config("AUDIO_OPTIONS.G1_NETWORK_INTERFACE", "eth1")

            # 创建音频编解码器实例
            self.audio_codec = AudioCodec(use_g1_audio=use_g1_audio, g1_network_interface=g1_network_interface)
            await self.audio_codec.initialize()

            # 设置实时编码回调
            self.audio_codec.set_encoded_audio_callback(self._on_encoded_audio)

            # 如果使用G1音频设备，设置初始音量和LED颜色
            if use_g1_audio and self.audio_codec.is_g1_available():
                g1_volume = self.config.get_config("AUDIO_OPTIONS.G1_VOLUME", 85)
                self.audio_codec.set_g1_volume(g1_volume)
                
                g1_led_color = self.config.get_config("AUDIO_OPTIONS.G1_LED_COLOR", {"R": 0, "G": 255, "B": 0})
                self.audio_codec.set_g1_led_color(
                    g1_led_color.get("R", 0),
                    g1_led_color.get("G", 255),
                    g1_led_color.get("B", 0)
                )
                
                logger.info(f"G1音频设备已启用: {self.audio_codec.get_audio_device_info()}")
    ...
    def _on_encoded_audio(self, encoded_data: bytes):
        ...
        """
        try:
            # 只有当WebSocket连接已建立且音频通道打开时才发送数据
            if (self.protocol
                    and getattr(self.protocol, 'connected', False)
                    and self.protocol.is_audio_channel_opened()
                    and not getattr(self, '_transitioning', False)):
                
                # logger.info(f"准备发送音频数据: {len(encoded_data)}字节, 设备状态: {self.device_state}")
                ...
                if not self.protocol:
                    logger.warning("协议未初始化，跳过音频发送")
                elif not getattr(self.protocol, 'connected', False):
                    # 这个警告只在第一次出现时打印，避免日志过多
                    if not getattr(self, '_connection_warning_shown', False):
                        logger.warning("WebSocket连接未建立，音频数据将被丢弃")
                        self._connection_warning_shown = True
                elif not self.protocol.is_audio_channel_opened():
                    logger.warning("音频通道未打开，跳过音频发送")
                elif getattr(self, '_transitioning', False):
                    logger.warning("设备状态转换中，跳过音频发送")
    ...
    def _schedule_audio_send(self, encoded_data: bytes):
        ...
        try:
            ...
            if (self.protocol
                    and self.protocol.is_audio_channel_opened()):
                # logger.info(f"实际发送音频数据: {len(encoded_data)}字节")
                # 创建异步任务发送音频数据
                asyncio.create_task(self.protocol.send_audio(encoded_data))
            else:
                logger.warning("音频通道已关闭，取消音频发送")
    ...
    async def _on_wake_word_detected(self, wake_word, full_text):
        ...
        唤醒词检测回调.
        """
        logger.info(f"检测到唤醒词: {wake_word}, 当前设备状态: {self.device_state}")

        # 如果正在说话，先打断
        if self.device_state == DeviceState.SPEAKING:
            logger.info("设备正在说话，准备打断...")
            await self.abort_speaking(AbortReason.WAKE_WORD_DETECTED)
        ...
        if not self.protocol.connected:
            logger.info("设备未连接，开始连接服务器...")
            await self._set_device_state(DeviceState.CONNECTING)
            
            logger.info("尝试连接WebSocket服务器...")
        ...
        # 打开音频通道（如果未打开）
        if not self.protocol.is_audio_channel_opened():
            logger.info("尝试打开音频通道...")
            if not await self.protocol.open_audio_channel():
                logger.error("打开音频通道失败")
                await self._set_device_state(DeviceState.IDLE)
                return
            logger.info("音频通道打开成功")

        # 发送唤醒词（短文本，服务器会自动回复欢迎语）
        logger.info("发送唤醒词到服务器...")
    ...
    async def handle_user_input(self, input_text: str):
        """处理用户输入的文本（语音转文字后的值）"""
        # 新增：检测“握手”关键词
        if "握手" in input_text.strip():
            # 调用MCP工具
            from src.mcp.mcp_server import get_mcp_server  # 导入MCP服务器
            mcp_server = get_mcp_server()
            try:
                # 执行握手工具
                result = await mcp_server.call_tool("self.handshake.trigger", {})
                # 输出结果（用于TTS播报或显示）
                return f"握手操作已执行：{result}"
            except Exception as e:
                return f"握手操作失败：{str(e)}"

        if ("比心" in input_text.strip()
                or "比个心" in input_text.strip()
                or "笔芯" in input_text.strip()):
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                # 调用比心工具
                result = await mcp_server.call_tool("self.bixin.trigger", {})
                return f"比心操作已执行：{result}"
            except Exception as e:
                return f"比心操作失败：{str(e)}"
        # 3. 拥抱（ID=3）
        if "拥抱" in input_text.strip() or "抱一下" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.yongbao.trigger", {})  # ID=3
                return f"拥抱操作已执行：{result}"
            except Exception as e:
                return f"拥抱操作失败：{str(e)}"

        # 4. 你好（ID=6）
        if "你好" in input_text.strip() or "hello" in input_text.strip().lower():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.nihao.trigger", {})  # ID=6
                return f"问候操作已执行：{result}"
            except Exception as e:
                return f"问候操作失败：{str(e)}"

        # 5. 亲一下（ID=7）
        if "亲一下" in input_text.strip() or "亲亲" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.qin.trigger", {})  # ID=7
                return f"亲吻操作已执行：{result}"
            except Exception as e:
                return f"亲吻操作失败：{str(e)}"

        # 新增：响应“看一下”“有什么”
        if "看一下" in input_text.strip() or "有什么" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.check.trigger", {})
                return f"查看操作已执行：{result}"
            except Exception as e:
                return f"查看操作失败：{str(e)}"

        # 新增：响应“我是谁”“认识我”
        if "看一看我是谁" in input_text.strip() or "启动人脸识别" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.identify.trigger", {})
                return f"身份识别操作已执行：{result}"
            except Exception as e:
                return f"身份识别操作失败：{str(e)}"

        # 新增：响应“厕所”“卫生间”
        if "厕所" in input_text.strip() or "卫生间" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.toilet.trigger", {})
                return f"卫生间查询结果：{result}"
            except Exception as e:
                return f"卫生间查询失败：{str(e)}"

        if "电梯" in input_text.strip() or "楼梯" in input_text.strip():
            from src.mcp.mcp_server import get_mcp_server
            mcp_server = get_mcp_server()
            try:
                result = await mcp_server.call_tool("self.elevator_stairs.trigger", {})
                return f"电梯/楼梯查询结果：{result}"
            except Exception as e:
                return f"电梯/楼梯查询失败：{str(e)}"
