...
# 用户ID → 专属动作 (覆盖默认的挥手动作)
# None 表示走默认逻辑 (execute_wave)
USER_ID_TO_ACTION = {
    "huyifan": "shake hand",  # 胡一帆: 执行握手
}
...
def execute_wave():
    ...
    if not (HAS_ARM_CLIENT and _arm_client is not None):
        print("[挥手] 机械臂未就绪, 跳过动作")
        return False
    try:
        print("🤖 执行: 挥手 (high wave)")
        _arm_client.ExecuteAction(action_map.get("high wave"))
        _time.sleep(2.0)
        _arm_client.ExecuteAction(action_map.get("release arm"))
        print("✅ 挥手完成 + 手臂已复位")
        return True
    except Exception as e:
        print(f"❌ 挥手动作失败: {e}")
...
def execute_handshake():
    """执行握手动作 (shake hand → 等待 → release arm)"""
    import time as _time
    if not (HAS_ARM_CLIENT and _arm_client is not None):
        print("[握手] 机械臂未就绪, 跳过动作")
        return False
    try:
        print("🤖 执行: 握手 (shake hand)")
        _arm_client.ExecuteAction(action_map.get("shake hand"))
        _time.sleep(2.0)
        _arm_client.ExecuteAction(action_map.get("release arm"))
        print("✅ 握手完成 + 手臂已复位")
        return True
...
def execute_action_for_user(user_id):
    """根据 user_id 派发对应的迎接动作 (默认挥手, 特定用户握手)"""
    action_name = USER_ID_TO_ACTION.get(user_id)
    if action_name == "shake hand":
        return execute_handshake()
    # 默认: 挥手
    return execute_wave()
    ...
    if killed:
        ...
        for script in killed:
            try:
                ...
                pass
        time.sleep(1.5)  # 再等待 1.5 秒, 让 USB 设备完全释放
    else:
        time.sleep(0.5)
...
def _capture_photo_realsense():
    ...
    # 前置清理: 避免第一次必失败 (相机被占用时 wait_for_frames 会超时 5 秒)
    print("[相机] 拍照前清理可能占用相机的进程...")
...
def _try_capture_realsense():
    """实际执行 RealSense 拍照 (含相机预热, 避免首帧过暗无人脸)"""
    ...
    try:
        ...
        # 相机预热: 丢弃前若干帧, 让曝光/白平衡稳定
        # (相机刚启动时画面可能过暗, 导致百度API报 "pic not has face")
        for i in range(15):
...
def main():
    ...
    try:
        ...
        if result.get("error_code") == 0:
            ...
            print(greeting)
            # 根据 user_id 派发迎接动作 (huyifan=握手, 其他=挥手)
            execute_action_for_user(user_id)
            # 然后语音播报 "你好，{name}！"
            speak_text(greeting)
        else:
