...
import time
import threading
import pygame  # 导入pygame音频库
import logging

# pip install websockets==11.0.3
# 安装pygame: pip install pygame -i https://mirrors.aliyun.com/pypi/simple/
...
def play_audio_after_delay(delay, audio_path):
    """延迟指定时间后播放MP3音频文件（使用pygame）"""
    try:
        # 等待指定秒数（25秒）
        time.sleep(delay)


        # 初始化pygame音频系统（仅初始化一次）
        if not pygame.mixer.get_init():
            pygame.mixer.init()

        # 加载音频文件
        logger.info(f"准备播放音频: {audio_path}")
        ...
        logger.error(f"音频播放失败（格式不支持或文件损坏）: {e}")
...
def parse_args():
    ...
    parser.add_argument(
        "--audio-path",
        default="/home/unitree/tang/WK/PythonProject/py-xiaozhi-main/111.mp3",
        help="指定要播放的MP3文件路径，默认是当前目录下的assistant_start.mp3",
    )
...
async def main():
    ...
    # 启动线程，25秒后播放MP3
    logger.info(f"启动音频提示线程，将在25秒后播放提示: {args.audio_path}")
    audio_thread = threading.Thread(
        target=play_audio_after_delay,
        args=(10, args.audio_path),  # 25秒延迟，音频文件路径
