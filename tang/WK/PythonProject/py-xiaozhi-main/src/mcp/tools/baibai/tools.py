...
async def baibai_function(args: dict) -> str:
    """
    触发拜拜动作（ID=4），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：high wave（拜拜）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=4动作（拜拜）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or f"ID=4动作（拜拜）执行成功（无输出）"
        return f"ID=4动作（拜拜）执行成功：{output}"
