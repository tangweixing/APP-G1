...
async def jujue_function(args: dict) -> str:
    """
    触发拒绝动作（ID=13），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：reject（拒绝）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=13动作（拒绝）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or f"ID=13动作（拒绝）执行成功（无输出）"
        return f"ID=13动作（拒绝）执行成功：{output}"
