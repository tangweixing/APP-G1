...
async def jizhang_function(args: dict) -> str:
    """
    触发击掌动作（ID=2），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：high five（击掌）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=2动作（击掌）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or "ID=2动作（击掌）执行成功（无输出）"
        return f"ID=2动作（击掌）执行成功：{output}"
