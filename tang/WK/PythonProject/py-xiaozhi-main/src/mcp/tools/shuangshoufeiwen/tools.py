...
async def shuangshoufeiwen_function(args: dict) -> str:
    """
    触发双手飞吻动作（ID=15），拉起指定路径的g1_arm_action_example.py脚本
    对应动作：two-hand kiss（双手飞吻）
    ...
    try:
        ...
        # 记录脚本输出到日志
        logger.info(f"ID=15动作（双手飞吻）执行输出: {result.stdout.strip()}")

        # 构造返回结果
        output = result.stdout.strip() or f"ID=15动作（双手飞吻）执行成功（无输出）"
        return f"ID=15动作（双手飞吻）执行成功：{output}"
