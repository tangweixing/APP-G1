...
class WakeWordDetector:
    ...
    async def _process_audio(self):
        ...
        try:
            ...
            if self._debug_counter % 50 == 0:  # 每50次打印一次
                logger.debug(f"唤醒词检测：已接收 {self._debug_counter} 次音频数据，长度: {len(data)} 字节")
    ...
    async def _process_audio_data(self, data):
        ...
        try:
            ...
            if self._audio_debug_counter % 100 == 0:
                logger.debug(f"唤醒词音频处理：已处理 {self._audio_debug_counter} 次，数据长度: {len(data)} 字节")

            # 处理完整识别结果
            if self.recognizer.AcceptWaveform(data):
                result = json.loads(self.recognizer.Result())
                if text := result.get("text", "").strip():
                    logger.debug(f"Vosk完整识别结果: {text}")  # 调试日志
                    # 过滤过短的文本以减少误触发
                    if len(text) >= 3:
            ...
            if self._partial_check_counter % 3 == 0:
                ...
                if partial and len(partial) >= 3:
                    logger.debug(f"Vosk部分识别结果: {partial}")  # 调试日志
