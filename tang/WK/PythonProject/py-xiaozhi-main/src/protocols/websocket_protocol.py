...
class WebsocketProtocol(Protocol):
    ...
    async def connect(self) -> bool:
        ...
        logger.info(f"开始连接WebSocket: {self.WEBSOCKET_URL}")
        logger.info(f"使用Headers: {self.HEADERS}")
        ...
        try:
            ...
            use_g1_audio = config.get_config("AUDIO_OPTIONS.USE_G1_AUDIO", False)
            
            # G1设备使用16kHz，其他设备使用配置的采样率
            sample_rate = AudioConfig.INPUT_SAMPLE_RATE if use_g1_audio else AudioConfig.OUTPUT_SAMPLE_RATE
                ...
                try:
                    if isinstance(message, str):
                        try:
                            ...
                            logger.info(f"收到服务器JSON消息: {msg_type}, 内容: {data}")
                            if msg_type == "hello":
                                # 处理服务器 hello 消息
                        ...
                        # 二进制消息，可能是音频
                        # logger.info(f"收到二进制消息: {len(message)}字节, 前20字节: {message[:20].hex()}")
    ...
    async def send_audio(self, data: bytes):
        ...
        if not self.is_audio_channel_opened():
            logger.warning("音频通道未打开，无法发送音频数据")
            return

        try:
            # logger.info(f"WebSocket发送音频数据: {len(data)}字节")
