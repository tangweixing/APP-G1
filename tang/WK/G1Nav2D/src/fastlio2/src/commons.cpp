...
void LivoxData::callback(const livox_ros_driver2::CustomMsg::ConstPtr &msg)
{
    ...
    fastlio::PointCloudXYZI::Ptr ptr1(new fastlio::PointCloudXYZI());
    livox2pcl(msg, ptr, ptr1);
    // ROS_INFO("size: %lu",ptr->size());
    buffer_filter.push_back(ptr);
    buffer.push_back(ptr1);
    time_buffer.push_back(last_timestamp);
}

void LivoxData::livox2pcl(const livox_ros_driver2::CustomMsg::ConstPtr &msg, fastlio::PointCloudXYZI::Ptr &out_filter, fastlio::PointCloudXYZI::Ptr &out)
{
    int point_num = msg->point_num;
    out_filter->clear();
    out->clear();
    out_filter->reserve(point_num / filter_num + 1);
    out->reserve(point_num);
    uint valid_num = 0;
...
{
    if (imu_data.buffer.empty() || livox_data.buffer_filter.empty())
        return false;

    if (!lidar_pushed)
    {
        lidar = livox_data.buffer_filter.front();
        lidar_org = livox_data.buffer.front();
        // std::cout << "Lidar size: " << lidar->size() << std::endl;
    ...
    livox_data.buffer_filter.pop_front();
